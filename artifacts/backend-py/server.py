...
def _seed_demo(db):
    ...
    if not db.execute("SELECT 1 FROM employees LIMIT 1").fetchone():
        employees = [
            (1,"Зарина М.","HR менеджер","#9b72ff","#9b72ff20",1,0,3500000),
            (2,"Усмон Н.","Финансовый менеджер","#1a7a3c","#1a7a3c20",0,1,4500000),
            (3,"Нилуфар С.","Менеджер продаж","#d68910","#d6891020",0,0,2800000),
            (4,"Камол Р.","Прораб","#1a5fb4","#1a5fb420",0,0,3200000),
            (5,"Фаррух Т.","Дизайнер","#c0392b","#c0392b20",0,0,2500000),
            (6,"Бехруз А.","IT специалист","#6c3483","#6c348320",1,1,3800000),
        ]
        db.executemany("INSERT OR IGNORE INTO employees(id,name,role,color,bg,is_hr,is_admin,salary) VALUES(?,?,?,?,?,?,?,?)", employees)

    # Задачи
    if not db.execute("SELECT 1 FROM tasks LIMIT 1").fetchone():
        tasks = [
            ("Отчёт Q2 2025","Квартальный финансовый анализ",2,"Высокий","Финансы","2025-05-05","В работе",65),
            ("Смета объекта Мира 12","Детальная смета строительства",4,"Высокий","Строительство","2025-04-30","Новая",0),
            ("Набор — 3 вакансии на склад","HR подбор персонала",1,"Средний","HR","2025-04-29","На проверке",80),
            ("Договор поставки красок","Согласовать условия договора",3,"Высокий","Юридический","2025-04-26","Заблокирована",20),
            ("Настройка 1С — склад","Интеграция складского учёта",6,"Высокий","IT","2025-05-02","В работе",50),
            ("Каталог мебели 2025","Фото и описания новых позиций",5,"Средний","Мебель","2025-05-08","В работе",35),
        ]
        db.executemany("INSERT INTO tasks(name,description,emp_id,priority,category,due_date,status,progress) VALUES(?,?,?,?,?,?,?,?)", tasks)

    # Офис
    if not db.execute("SELECT 1 FROM offices LIMIT 1").fetchone():
        db.execute("INSERT INTO offices(name,lat,lng,radius) VALUES('Главный офис',41.2995,69.2401,200)")

    # Дебиторы
    if not db.execute("SELECT 1 FROM debtors LIMIT 1").fetchone():
        debtors = [
            ("D001","ООО СтройСнаб","123456789",3,48.5,45,"2025-03-10","2025-01-10","2025-02-15","negotiating"),
            ("D002","ИП Рашидов А.К.","987654321",3,12.3,35,"2025-03-20","2025-02-20","2025-03-01","promised"),
            ("D003","АО ЛКМ-Трейд","456789123",3,87.2,99,"2025-01-15","2024-12-15","2025-01-20","legal"),
            ("D004","ТОО МебельГрупп","321654987",4,23.7,23,"2025-04-01","2025-03-01","2025-03-15","negotiating"),
            ("D005","ООО КраскиОпт","654321098",4,9.8,4,"2025-04-20","2025-03-20","2025-04-01","promised"),
            ("D006","ЗАО ФинансГрупп","789012345",4,156.0,145,"2024-12-01","2024-11-01","2024-12-01","dispute"),
            ("D007","ИП Каримов Б.","112233445",5,5.6,0,"2025-04-25","2025-04-01","2025-04-10","promised"),
            ("D008","ООО АлмазСтрой","556677889",5,34.1,68,"2025-02-15","2025-01-15","2025-02-28","partial"),
        ]
        db.executemany("INSERT OR IGNORE INTO debtors(id,name,inn,manager_id,debt,overdue_days,due_date,invoice_date,last_payment,status) VALUES(?,?,?,?,?,?,?,?,?,?)", debtors)

    # Склад
    if not db.execute("SELECT 1 FROM warehouse LIMIT 1").fetchone():
        items = [
            ("W001","Краска фасадная белая","KFB-001","Краски и ЛКМ",245,"кг",50,12500,"Склад №1","Tikkurila","22.04"),
            ("W002","Краска интерьерная бежевая","KIB-002","Краски и ЛКМ",18,"кг",30,11000,"Склад №1","Dulux","18.04"),
            ("W003","Грунтовка универсальная","GRU-003","Краски и ЛКМ",0,"л",20,8500,"Склад №1","Isomat","10.04"),
            ("W004","Диван угловой Comfort","DUC-001","Мебель",4,"шт",2,3200000,"Склад №2","Porta","20.04"),
            ("W005","Кресло офисное Premium","KOP-002","Мебель",12,"шт",5,890000,"Склад №2","Porta","19.04"),
            ("W006","Цемент М500","CEM-001","Стройматериалы",380,"кг",100,4500,"Склад №3","МестныйПроизв.","21.04"),
            ("W007","Арматура 12мм","ARM-012","Стройматериалы",850,"м",200,18000,"Склад №3","МестныйПроизв.","20.04"),
            ("W008","Кабель NYM 3x2.5","KAB-325","Электрика",0,"м",50,6500,"Склад №1","КитайИмпорт","05.04"),
            ("W009","Розетка двойная ABB","ROZ-ABB","Электрика",67,"шт",20,25000,"Склад №1","КитайИмпорт","17.04"),
        ]
        db.executemany("INSERT OR IGNORE INTO warehouse(id,name,sku,category,qty,unit,min_qty,price,warehouse_name,supplier,last_in) VALUES(?,?,?,?,?,?,?,?,?,?,?)", items)

    # Демо посещаемость — последние 45 рабочих дней
    if not db.execute("SELECT 1 FROM attendance LIMIT 1").fetchone():
        import random, math
        today = date.today()
        rows = []
        for emp_id in range(1, 7):
            for d in range(1, 46):
                ...
                if random.random() < 0.07:
                    rows.append((emp_id, ds, None, None, None, None, "absent", 0, 0, 0, 0))
                    continue
                late = random.random() < 0.18
                early = random.random() < 0.08
                in_min = 9*60 + (random.randint(5,35) if late else -random.randint(0,3))
                out_min = 18*60 + (-random.randint(15,35) if early else random.randint(0,25))
                lat = 41.2995 + (random.random()-0.5)*0.002
                lng = 69.2401 + (random.random()-0.5)*0.002
                status = "late" if late else ("early_out" if early else "present")
                late_m = max(0, in_min - 9*60)
                early_m = max(0, 18*60 - out_min)
                time_in  = f"{in_min//60:02d}:{in_min%60:02d}"
                time_out = f"{out_min//60:02d}:{out_min%60:02d}"
                rows.append((emp_id, ds, time_in, time_out, lat, lng, status, 1, 0, late_m, early_m))
        db.executemany("INSERT OR IGNORE INTO attendance(emp_id,date,time_in,time_out,lat,lng,status,auto_in,auto_out,late_min,early_min) VALUES(?,?,?,?,?,?,?,?,?,?,?)", rows)
